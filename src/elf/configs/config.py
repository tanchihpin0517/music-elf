...
class Config:
    ...
    max_train_examples: int = None  # Optional cap for smoke tests
    max_length: int = 128
    ...
    modality: str = "text"  # "text" or "music"
    tokenizer_path: str = None  # miditok tokenizer json for music

    # Encoder
    encoder_model_name: str = "t5-small"
    encoder_checkpoint: str = None
    encoder_type: str = "t5_pretrained"  # "t5_pretrained" or "t5_scratch"
...
def resolve_sampling_configs_path(sampling_configs_path: str, config_yaml_path: str) -> str:
    """Resolve sampling_configs_path relative to the training config file."""
    if not sampling_configs_path or os.path.isabs(sampling_configs_path):
        return sampling_configs_path
    if os.path.isfile(sampling_configs_path):
        return sampling_configs_path

    cfg_dir = os.path.dirname(os.path.abspath(config_yaml_path))
    basename = os.path.basename(sampling_configs_path)
    candidates = [
        os.path.join(cfg_dir, sampling_configs_path),
        os.path.join(cfg_dir, basename),
        os.path.normpath(os.path.join(cfg_dir, "..", "sampling_configs", basename)),
    ]
...
def load_config_from_yaml(path: str) -> Config:
    ...
    if config.sampling_configs_path:
        config.sampling_configs_path = resolve_sampling_configs_path(
            config.sampling_configs_path, path,
        )
